...
def compare_scenarios(
    ...
    output_file: str = "comparacion_escenarios.xlsx"
) -> str:
    """
    ...
    for scenario in scenarios:
        ...
        results_list.append({
            ...
            "Tipo Final (%)": max(0, scenario["data"].interest_rate - calculator.calculate_total_bonus()),
            "Cuota Mensual (€)": results.monthly_payment_with_bonus if calculator.calculate_total_bonus() > 0 else results.monthly_payment_without_bonus,
            "Total Intereses (€)": results.total_interest_with_bonus if calculator.calculate_total_bonus() > 0 else results.total_interest_without_bonus,
            "Total a Pagar (€)": results.total_paid_with_bonus if calculator.calculate_total_bonus() > 0 else results.total_paid_without_bonus,
            "Costes Bonificaciones (€)": results.total_bonus_costs,
            "Coste Real Total (€)": (results.total_paid_with_bonus if calculator.calculate_total_bonus() > 0 else results.total_paid_without_bonus) + results.total_bonus_costs,
            "Ahorro Real (€)": results.real_savings if calculator.calculate_total_bonus() > 0 else 0,
            "¿Vale la pena?": "SÍ" if results.is_worth_it else "NO" if calculator.calculate_total_bonus() > 0 else "N/A"
        })
...
def sensitivity_analysis(
    ...
    while rate <= rate_range[1]:
        ...
        results_list.append({
            ...
            "Diferencia Cuota (€)": results.monthly_payment_without_bonus - results.monthly_payment_with_bonus,
...
def recommend_best_bonus_combination(
    ...
    for i in range(1, 2**n):
        ...
        data = MortgageData(
            ...
            interest_rate=mortgage_data.interest_rate,
            years=mortgage_data.years,
            payroll_bonus=total_bonus if "nomina" in combination or "payroll" in combination else 0.0,
            insurance_bonus=total_bonus if "seguros" in combination or "insurance" in combination else 0.0,
...
if __name__ == "__main__":
    ...
    scenarios = [
        ...
        },
        {
            ...
            "data": MortgageData(
                capital=200000, interest_rate=3.75, years=30,
                payroll_bonus=0.30, insurance_bonus=0.50,
    ...
    base_data = MortgageData(
        capital=200000, interest_rate=3.50, years=30,
        payroll_bonus=0.30, insurance_bonus=0.50,
        insurance_cost_monthly=45.0
    )
